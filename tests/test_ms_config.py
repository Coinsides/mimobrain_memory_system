...
def test_ms_config_defaults_paths(tmp_path: Path):
    ...
    cfg_path.write_text(
        json.dumps(
            {"vault_roots": {"default": str(tmp_path / "vault")}}, ensure_ascii=False
        ),
        encoding="utf-8",
    )

    cfg = load_config(cfg_path)
    assert cfg["raw_manifest_path"].endswith("manifests\\raw_manifest.jsonl") or cfg[
        "raw_manifest_path"
    ].endswith("manifests/raw_manifest.jsonl")
